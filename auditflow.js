...
(function () {
  ...
  // ── DEBUG LOGGING ────────────────────────────────────
  function debug(msg) {
    console.log(`[AuditFlow Debug] ${msg}`);
  ...
  // ── PAGE LOADER (RADICAL DESTRUCTION) ────────────────
  function hideLoader(reason = 'unknown') {
    debug(`hideLoader called: ${reason}`);
    ...
    if (loader) {
      ...
      setTimeout(() => { 
        ...
        loader.remove(); // Radikal entfernen
        debug('Loader removed from DOM');
      }, 350);
    }
  }

  // RADIKALER FALLBACK: Nach 1 Sekunde MUSS der Loader weg
  setTimeout(() => hideLoader('Hard Timeout Fallback'), 1000);
  window.addEventListener('load', () => hideLoader('Window Load Event'));
  document.addEventListener('DOMContentLoaded', () => hideLoader('DOMContentLoaded'));
